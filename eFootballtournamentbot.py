...
# Store the list of users waiting for approval (new data structure for better tracking)
waiting_approvals: Dict[int, Dict[str, int]] = {
    15: [],  # Users waiting for ₹15 tournament
    30: [],  # Users waiting for ₹30 tournament
    50: []   # Users waiting for ₹50 tournament
}
...
async def register_tournament_callback(update: Update, context: CallbackContext):
    ...
    # Add the user to the list of users waiting for approval
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    if action == "approve":
        ...
        # Fetch the waiting list for the specific tournament
        waiting_list = waiting_approvals[tournament_type]

        if len(waiting_list) == 0:
            # No users to approve
            await context.bot.send_message(chat_id=user_id, text="❌ No users in the waiting list.")
            return

        # Assign the first available slot for the user
        slot_assigned = False
        if TOURNAMENTS[tournament_type]["slots"].get(1) is None:
            # If Slot 1 is available, assign the user to Slot 1
            TOURNAMENTS[tournament_type]["slots"][1] = user_id
            slot_assigned = True
            slot = 1
        elif TOURNAMENTS[tournament_type]["slots"].get(2) is None:
            # If Slot 2 is available, assign the user to Slot 2
            TOURNAMENTS[tournament_type]["slots"][2] = user_id
            slot_assigned = True
            slot = 2
        else:
            # Both Slot 1 and Slot 2 are filled, place the user in Slot 1 (overwrite)
            TOURNAMENTS[tournament_type]["slots"][1] = user_id
            # Empty Slot 2 when Slot 1 is reassigned
            TOURNAMENTS[tournament_type]["slots"][2] = None
            slot_assigned = True
            slot = 1
        ...
        if slot_assigned:
            ...
            # Remove the user from the waiting list after approval
            ...
            await context.bot.send_message(chat_id=user_id, text="❌ All slots are filled. Please try again later.")
...
# Group update function - Updated to handle updated tournament slots

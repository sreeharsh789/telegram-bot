...
slots = []  # List to store registered players in the current slot
...
async def register_tournament_callback(update: Update, context: CallbackContext):
    ...
    # Find an empty slot
    empty_slot = next((slot for slot, user in tournament["slots"].items() if user is None), None)

    if empty_slot is None:
        await query.edit_message_text("All slots for this tournament are full. Try another!")
        return

    # Proceed with the registration process
    # Send QR code and notify admin
    ...
    await context.bot.send_message(
        ...
        text=(f"🛑 New Registration Request 🛑\n\n"
              ...
              f"⚽ <b>Slot:</b> {empty_slot}\n\n"
              f"Approve or Decline below."),
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Approve", callback_data=f"approve_{tournament_type}_{user_id}_{empty_slot}")],
